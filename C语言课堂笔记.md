...
+ **宏定义**
  ...
  + 预定义的宏
    ...
    + **条件式编译**
      ...
      | #ifndef  DEBUG   | 是否没定义DEBUG         |
      | #if  MAX_N  == 5 | 宏 MAX_N是否等于5       |
      | #elif MAX_N ==4  | 否则 宏MAX _N 是否等于4 |
      | #else            |                         |
      | #endif           |                         |
...
+ **字符串的相关操作**
  ...
  + \<string.h\>
    ...
    | 函数                    | 说明                                          |
    | ----------------------- | --------------------------------------------- |
    | strlen(str)             | 计算字符长度， 以\0作为结束符，并算做一个字符 |
    | strcmp(str1, str2)      | 字符串比较                                    |
    | strcpy(dest,src)        | 字符串拷贝                                    |
    | strncmp(str1, srt 2, n) | 字符串安全比较                                |
    | strncpy(str1,str2,n)    | 字符串的安全拷贝                              |
    | memcpy(str1, str2, n)   | 内存拷贝                                      |
    | memcmp(str1,str2,n)     | 内存比较                                      |
    | memset(str1, c , n)     | 内存设置 从地址str1 开始的往后n个字节设置为c  |
