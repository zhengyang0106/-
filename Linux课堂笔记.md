...
- 标准输入输出和错误  ---\>每个进程都会在内部打开三个文件

  + 0  --\>标准输入
  + １ --\>标准输出
  + ２ --\>标准错误输出
  + ｅｊ：
    1. ls . /etc1   1>  a    2>　&1  --->把标准输出　重定向到ａ　然后把标准错误输出１　
    2. 注意　１和　>　之间没有空格　　　

+ **权限**
  + chmod  修改ｒｗｘ
  + chown  修改所属权限　　chown  zhengyang:root  file   可以同时修改组
  + chogrp  修改组　　
  + u --文件所属者　　g -- 文件所属组　　  o  --- 其它用户　　
...
+ **whoami  与　who am i**
  + whoami   输出当前用户身份
  + who am i  输出用户真实身份　　比如　zhengyang 切换到　ｒｏｏｔ　　其真实身份任然是　zhengyang

+ **su**

  --- 切换用户　　　加　－　　会把bash 的　配置一起切换过去

+ echo $?

   －－输出上一条命令的返回值　例如　标注错误输出返回的是　２
...
+ **\` \`  命令替换符**
  ...
  ​      ---优先执行 \`  \`里的命令 然后当做参数 传给第一个命令  ej： ls   \` echo $/etc\`

+ **挂起 和 前后台**

  + command  &  --\>命令command 放到后台执行
    1. 结束 命令  pkill  command
    2. pkill  command 也是一个模糊匹配命令  可以结束相似的开头命令
  + ctrl + z   进程挂起
    1. jobs查看工作状态
  + bg  fg  
    1. bg  --\>放到后台执行
...
+ **bash 通配符**
  1. ​    ?      --\>  代表单个字符
  2. ​    \*      --\> 代表任意多个字符
  3. ​    [list]    --\>匹配list中任意单一字符
  4. ​    [!list]   --\>不匹配list中任意单一字符
  5. ​    {str1,str2,...}   -->匹配任意str
+ **管道  重定向**
  1.   \>  重定向符    命令 > 文件
  2.   \>\>  追加符    追加到文件的末尾 不会覆盖掉之前文件的内容
  3.   \<      把文件的内容 重定向到 命令 当做输出
  4.   \<\<   定义结束字符  cat  \>\>  a    \<\<123    定义123 为结束符   